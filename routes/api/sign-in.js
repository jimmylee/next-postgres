...
export default async (req, res) => {
  if (Strings.isEmpty(req.body.email)) {
    return res.status(500).send({ error: 'An e-mail address was not provided.' });
  }
  ...
  if (!user) {
    ...
    if (user.error) {
      return res.status(500).send({ error: 'We could not authenticate you (1).' });
    }
    ...
    if (phaseThree !== user.password) {
      return res.status(500).send({ error: 'We could not authenticate you (2).' });
    }
  }
  ...
  const token = JWT.sign({ user: user.id, email: user.email }, Credentials.JWT_SECRET);
